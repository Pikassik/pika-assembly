...
void pushc(struct ExecutorState& state, uint32_t constant) { PUSH(constant); }
void pushr(struct ExecutorState& state, size_t reg0) { PUSH(IREG(reg0)); }
void pushfc(struct ExecutorState& state, uint32_t constant) { PUSH(constant); }
void pushfr(struct ExecutorState& state, size_t reg0) { PUSH(TOINT(FREG(reg0))); }
...
void popr(struct ExecutorState& state, size_t reg0) { IREG(reg0) = TOP; POP; }
void popfr(struct ExecutorState& state, size_t reg0) { FREG(reg0) = TOFLOAT(TOP); POP; }
...
void inint(struct ExecutorState& state) { uint32_t in; scanf("%d", &in); PUSH(in); }
void inch(struct ExecutorState& state) { int32_t ch; ch = getchar(); PUSH(ch); }
void inf(struct ExecutorState& state) { float fl; scanf("%f", &fl); PUSH(TOINT(fl)); }
void outint(struct ExecutorState& state) { printf("%d", TOP); }
void outch(struct ExecutorState& state) { printf("%c", TOP); }
void outf(struct ExecutorState& state) { printf("%f", TOFLOAT(TOP)); }
